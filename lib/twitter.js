 ...
 */
Twitter.prototype.auth = function(mount, params) {
	...
	return function handle(req, res, next) {
		...
		// FIXME: handle a different frontend url scheme (eg. https proxy)
		var scheme = req.socket.secure ? 'https://' : 'http://',
			purl = url.parse(scheme + req.headers.host + req.url, true),
			pmount = url.parse(scheme + req.headers.host + mount, true);

		// Set the oauth_callback based on this request if it's not there already
		...
		// Begin OAuth transaction if we have no cookie
		} else if ( !twauth ) {
			...
			function(error, oauth_token, oauth_token_secret, oauth_authorize_url, params) {
				if ( error ) {
					...
					res.end();
					// Remember where we began the transaction
					returl = purl.href || returl;
					return;
				}
			});

		// Broken cookie, clear it and return to originating page
		// FIXME: include a "broken" destination option?
		} else {
