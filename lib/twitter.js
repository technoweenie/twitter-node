...
Twitter.prototype.stream = function(method, params, callback) {
	...
	if (method === 'user') {
		...
		if ( params.track && Array.isArray(params.track) ) {
		...
		if ( params.follow && Array.isArray(params.follow) ) {
	...
	var request = this.oauth.get(url + '?' + querystring.stringify(params),
		this.options.access_token_key,
		this.options.access_token_secret);
	...
	request.on('response', function(response) {
		...
		// FIXME: Somehow provide UPDATED chunks of the response when the stream ends, too
...
Twitter.prototype.login = function(mount, success) {
	...
	// Fix up the cookie secret if we have one
	// Doing it here ensures it's one done once
	// FIXME: means configurations that don't use login won't work...
...
Twitter.prototype.gatekeeper = function(failure) {
	...
	return function(req, res, next) {
		// We have a winner
		if ( self.cookie(req) ) return next();

		// I pity the fool!
		// FIXME: use above parameter to fail with: a) 401, b) redirect
